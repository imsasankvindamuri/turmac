...
class TURMAC:
        ...
        while self.program_counter < len(self.tokenized_code):
            ...
            if cmd[0] in list(self.stdlib_syntax_verify.keys()):
                ...
                if match:
                    ...
                    raise SyntaxError(
                        f"Uh Oh... Expected syntax '{self.stdlib_syntax_verify[cmd[0]]}', received syntax '{' '.join(cmd)}'"
                    )
            else:
                self.eval_rpn(cmd)
            ...
            print(f"Data Pointer    : {self.data_pointer}")
    ...
    def eval_rpn(self, toks: list[str] = [""]) -> None:
        ...
        if len(stack) != 1:
            raise Exception(f"""Uh Oh... Something went wrong at Program Counter = {self.program_counter} :
    ...
    def tokenize(self) -> list[str]:
        code_lines = [
            line.strip()
            for line in self.raw_code.split("\n")
    ...
    def state(self) -> None:
        print(
            f"/// Memory Tape = {self.memory_tape} | Program Counter = {self.program_counter} | Data Pointer = {self.data_pointer} ///"
        )
    ...
    def set_flag_registry(self) -> None:
        ...
        while line_number < len(self.tokenized_code):
            ...
            if cmd[0] == "flag":
                ...
                if not match:
                    raise SyntaxError(
                        f"Uh Oh... Expected syntax '{self.stdlib_syntax_verify[cmd[0]]}', received syntax '{command}'"
                    )
                else:
    ...
    def _stdin(self) -> None:
        try:
            val = int(
                input(
                    f"Please provide input for Memory Location {self.data_pointer}:\n"
                )
            )
            self.memory_tape[self.data_pointer] = val
        except ValueError:
            raise ValueError(
                "TURMAC is a mono-typed language and only accepts integers."
            )
